/*
  ...
  1. New Tables
    - `user_tokens` - Central table for tracking user token balances
      - `id` (uuid, primary key)
      - `user_id` (uuid, foreign key to auth.users)
      - `total_tokens` (integer, total tokens allocated)
      - `tokens_remaining` (integer, tokens currently available)
      - `tokens_used` (integer, tokens used so far)
      - `last_updated` (timestamp)
      - `created_at` (timestamp)

    - `token_transactions` - Detailed log of all token transactions
      - `id` (uuid, primary key)
      - `user_id` (uuid, foreign key to auth.users)
      - `session_id` (uuid, optional foreign key to coaching_sessions)
      - `amount` (integer, positive for additions, negative for deductions)
      - `transaction_type` (enum: 'purchase', 'usage', 'refund', 'renewal', 'bonus')
      - `description` (text, human-readable description)
      - `reference_id` (text, optional reference to external system)
      - `created_at` (timestamp)

  2. Functions
    - `get_user_tokens` - Get current token balance for a user
    - `add_user_tokens` - Add tokens to a user's balance
    - `use_user_tokens` - Deduct tokens from a user's balance
    - `sync_user_tokens` - Sync token balance with subscription data

  3. Triggers
    - Auto-update tokens_used when tokens_remaining changes
    - Auto-create token record for new users
    - Auto-sync tokens when subscription changes
...
CREATE OR REPLACE FUNCTION sync_user_tokens(p_user_id uuid)
RETURNS boolean AS $$
DECLARE
...
-- Create function to sync all users' tokens
CREATE OR REPLACE FUNCTION sync_all_user_tokens()
RETURNS integer AS $$
DECLARE
  v_user_id uuid;
  v_count integer := 0;
BEGIN
  ...
  LOOP
    ...
    IF sync_user_tokens(v_user_id) THEN
      v_count := v_count + 1;
    END IF;
  END LOOP;
...
COMMENT ON TABLE user_tokens IS 'Central table for tracking user token balances';
COMMENT ON TABLE token_transactions IS 'Detailed log of all token transactions';
COMMENT ON FUNCTION get_user_tokens IS 'Get current token balance for a user';
COMMENT ON FUNCTION add_user_tokens IS 'Add tokens to a user''s balance and record transaction';
COMMENT ON FUNCTION use_user_tokens IS 'Deduct tokens from a user''s balance and record transaction';
COMMENT ON FUNCTION sync_user_tokens IS 'Sync token balance with subscription data';
COMMENT ON FUNCTION sync_all_user_tokens IS 'Sync token balances for all users';
COMMENT ON VIEW user_token_summary IS 'User-specific view of token balance and subscription info';
COMMENT ON VIEW user_token_transactions IS 'User-specific view of token transaction history';
...
DECLARE
  v_count integer;
BEGIN
  SELECT sync_all_user_tokens() INTO v_count;
  RAISE NOTICE 'Synced tokens for % users', v_count;
END $$;