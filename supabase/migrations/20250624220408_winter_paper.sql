/*
  # Fix Unified Coaches Migration

  1. Changes
    - Properly migrate data from human_coaches to ai_coaches table
    - Handle missing columns gracefully
    - Ensure both Natalie and Fatten are properly migrated
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'human_coaches') THEN
    ...
    LOOP
      -- Check if cal_com_link column exists in human_coaches table
      IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'human_coaches' AND column_name = 'cal_com_link'
      ) THEN
        EXECUTE format('SELECT cal_com_link FROM human_coaches WHERE id = %L', human_coach_record.id) INTO cal_link;
      ELSE
        cal_link := 'https://cal.com/' || lower(replace(human_coach_record.name, ' ', '-'));
      END IF;
...
-- Ensure Natalie Sejean exists with correct data
INSERT INTO ai_coaches (
  name, 
  specialty, 
  description, 
...
-- Drop human_coaches table after successful migration
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'human_coaches') THEN
    -- First, remove any foreign key constraints that might reference human_coaches
    ALTER TABLE coaching_sessions DROP CONSTRAINT IF EXISTS coaching_sessions_human_coach_id_fkey;
    
    -- Drop the table
    DROP TABLE human_coaches CASCADE;
  END IF;
END $$;

-- Remove human_coach_id column from coaching_sessions since we're using unified table
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
