/*
  ...
  1. New Tables
    - `credit_transactions` - Track all credit additions/deductions
      - `id` (uuid, primary key)
      - `user_id` (uuid, foreign key to auth.users)
      - `transaction_type` (enum: 'purchase', 'renewal', 'usage', 'refund')
      - `credits_amount` (integer, positive for additions, negative for deductions)
      - `description` (text, description of the transaction)
      - `stripe_subscription_id` (text, optional reference to Stripe subscription)
      - `stripe_invoice_id` (text, optional reference to Stripe invoice)
